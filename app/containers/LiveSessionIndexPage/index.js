...
import makeSelectLiveSessionIndexPage from './selectors';
...
export function LiveSessionIndexPage({ match, location, dispatch }) {
  useInjectReducer({ key: 'liveSessionIndexPage', reducer });
  useInjectSaga({ key: 'liveSessionIndexPage', saga });

  const user = reactLocalStorage.getObject('user');

  const AllLiveSession = () => {
    ...
    const getFeeds = (refresh = false) => {
      ...
      if (hasMore) {
        ...
        axiosInstance
          .get(
            `api/live-session?page=${page}&per_page=20&user_id=${
              user.user_id
            ...
            <InfiniteScroll
              ...
              endMessage={
                !feeds.length && (
                  ...
                  >
                    <b>No Available Live Sessions.</b>
                  </div>
                )
              }
              pullDownToRefresh
                  ...
                  >
                    <InterestFeedCardComponent
                      feed={{
                        ...
                        is_joined_live_session: feed.is_joined_live_session,
                      }}
            ...
            <InfiniteScroll
              ...
              endMessage={
                !feeds.length && (
                  ...
                  >
                    <b>You are not participating in any Live Sessions yet.</b>
                    <div
                      className="h3 cyan-text"
                      style={{
                        textAlign: 'center',
            ...
            >
              {feeds.map((feed, i) => (
                <div key={i}>
                  ...
                  <div
                    onClick={() => {
                      dispatch(push(`live-sessions/${feed.live_id}?tab=2`));
                    }}
                  >
                    <InterestFeedCardComponent
                      feed={{
  ...
  const PageWrapper = ({ tab }) =>
    tab === 'all-lives' ? <AllLiveSession /> : <MyLiveSession />;

  return (
    <div>
      <Helmet>
        <title>Live Sessions - Live to Give</title>
...
LiveSessionIndexPage.propTypes = {
  dispatch: PropTypes.func.isRequired,
};

const mapStateToProps = createStructuredSelector({
  liveSessionIndexPage: makeSelectLiveSessionIndexPage(),
});
