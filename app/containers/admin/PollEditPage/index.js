...
import makeSelectPollEditPage from './selectors';
...
export function PollEditPage({ match, dispatch }) {
  useInjectReducer({ key: 'pollEditPage', reducer });
  useInjectSaga({ key: 'pollEditPage', saga });
  ...
  const getData = () => {
    axiosInstance
      .get(`api/admin/all-polls/${match.params.id}`)
      .then(res => {
  ...
  const Form = ({ poll }) => {
    ...
    const [option1, setOption1] = useState(poll.option_one);
    const [option2, setOption2] = useState(poll.option_two);
    const [option3, setOption3] = useState(poll.option_three);
    const [option4, setOption4] = useState(poll.option_four);
    const [startDateTime, setStartDateTime] = useState(
      moment.utc(poll.started_at),
    );
    const [endDateTime, setEndDateTime] = useState(moment.utc(poll.ended_at));

    const [image] = useState(poll.image_cover);
        ...
        <div className="white-text">
            ...
            {image || tempPic ? (
              <ImageUploadSection justify="center">
                <img
                  src={
                    tempPic ||
                    `${process.env.IMAGE_URL_PREFIX}/poll/${
                      match.params.id
                    }/${image}`
                  }
                  alt="profile-pic"
          ...
          <ConfirmationPopupComponent
            ...
            dismissModal={() => {
              ...
              dispatch(push(`../../../admin/polls/${pollData.poll_id}`));
            }}
...
const mapStateToProps = createStructuredSelector({
  pollEditPage: makeSelectPollEditPage(),
});
