...
import makeSelectMeetupEditPage from './selectors';
...
export function MeetupEditPage({ match, dispatch }) {
  useInjectReducer({ key: 'meetupEditPage', reducer });
  useInjectSaga({ key: 'meetupEditPage', saga });
  ...
  const getData = async () => {
    const request1 = await axiosInstance
      .get(`api/admin/all-meetups/${match.params.id}`)
      .then(res => {
  ...
  const Form = ({ meetup }) => {
    ...
    const [htmlContent, setHtmlContent] = useState(meetup.html_content);
    const [notification, setNotification] = useState(
      meetup.notification_message,
    );
    const [interest, setInterest] = useState(
      interestList.find(i => i.id === meetup.interest_id).name,
    );
    const [slots, setSlots] = useState(meetup.slots);
    const [bcoin, setBcoin] = useState(meetup.bcoin_reward || '');
    const [extraInfo, setExtraInfo] = useState(meetup.additional_details || '');
    const [hostName, setHostName] = useState(meetup.host_name || '');
    const [hostEmail, setHostEmail] = useState(meetup.host_email || '');
    const [meetingLink, setMeetingLink] = useState(
      meetup.virtual_room_link || '',
    );
    const [recordingLink, setRecordingLink] = useState('');
    const [startDateTime, setStartDateTime] = useState(
      moment.utc(meetup.started_at),
    );
    const [endDateTime, setEndDateTime] = useState(moment.utc(meetup.ended_at));
    const [registrationEnd, setRegistrationEnd] = useState(
      moment.utc(meetup.registration_ended_at),
    );
    const [featured, setFeatured] = useState(meetup.is_featured);

    const [image] = useState(meetup.image_cover);
    ...
    const [editorState, setEditorState] = useState(() => {
      if (meetup.html_content) {
        const blocksFromHTML = convertFromHTML(meetup.html_content);
    ...
    const submit = () => {
      const params = {
        meetup_id: meetupData.meetup_id,
        title,
        ...
        <div className="white-text">
            ...
            {image || tempPic ? (
              <ImageUploadSection justify="center">
                <img
                  src={
                    tempPic ||
                    `${process.env.IMAGE_URL_PREFIX}/meetup/${
                      match.params.id
                    }/${image}`
                  }
                  alt="profile-pic"
          ...
          <ConfirmationPopupComponent
            ...
            dismissModal={() => {
              ...
              dispatch(push(`../../../admin/meetups/${meetupData.meetup_id}`));
            }}
            title="Update Successful"
            message="The meetup has been updated!"
            actionRequire={false}
          />
        </div>
      </>
...
MeetupEditPage.propTypes = {
  dispatch: PropTypes.func.isRequired,
};

const mapStateToProps = createStructuredSelector({
  meetupEditPage: makeSelectMeetupEditPage(),
});
