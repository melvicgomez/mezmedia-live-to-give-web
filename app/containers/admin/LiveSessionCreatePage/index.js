...
import makeSelectLiveSessionCreatePage from './selectors';
...
export function LiveSessionCreatePage({ match, dispatch }) {
  useInjectReducer({ key: 'liveSessionCreatePage', reducer });
  useInjectSaga({ key: 'liveSessionCreatePage', saga });
  ...
  const submit = () => {
    ...
    axiosInstance
      ...
      .then(res => {
        setLoading(false);
        dispatch(replace(`../../admin/live-sessions/${res.data.data.live_id}`));
      })
      .catch(() => {
        setLoading(false);
      });
...
LiveSessionCreatePage.propTypes = {
  dispatch: PropTypes.func.isRequired,
};

const mapStateToProps = createStructuredSelector({
  liveSessionCreatePage: makeSelectLiveSessionCreatePage(),
});
