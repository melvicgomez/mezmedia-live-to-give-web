...
import makeSelectLiveSessionListPage from './selectors';
...
export function LiveSessionListPage({ match, dispatch }) {
  useInjectReducer({ key: 'liveSessionListPage', reducer });
  useInjectSaga({ key: 'liveSessionListPage', saga });
  ...
  const columns = [
    {
      ...
      key: 'liveId',
      render: text => (
        <a className="white-text" href={`/admin/live-sessions/${text}`}>
          {text}
        </a>
      ),
    },
  ...
  const getList = (page, search = '', filter = 1, sorter = null) => {
    let api = `api/admin/all-live-sessions?page=${page}&search=${
        ...
        topTab={
          <Row style={{ width: '100%' }} justify="space-between">
            ...
            <PrimaryButtonComponent
              ...
              label="Create"
              onClick={() => dispatch(push('../../admin/live-session/create'))}
              iconRight={false}
            />
          </Row>
        }
...
LiveSessionListPage.propTypes = {
  dispatch: PropTypes.func.isRequired,
};

const mapStateToProps = createStructuredSelector({
  liveSessionListPage: makeSelectLiveSessionListPage(),
});
