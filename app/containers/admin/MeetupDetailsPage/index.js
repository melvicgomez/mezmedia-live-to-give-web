...
export function MeetupDetailsPage({ match, location, dispatch }) {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`api/admin/all-meetups/${meetupId}`)
      .then(res => {
  ...
  const getParticipants = (refresh = false) => {
    ...
    if (hasMore) {
      axiosInstance
        .get(`api/meetup/participants/${meetupId}?page=${page}`)
        .then(res => {
  ...
  const publish = () => {
    if (!publishLoading) {
      axiosInstance
        .post(`api/admin/all-meetups/publish/${meetupId}?action=publish`)
        .then(res => {
          meetupData.published_at = res.data.data.published_at;
  ...
  const unpublish = () => {
    if (!publishLoading) {
      axiosInstance
        .post(`api/admin/all-meetups/publish/${meetupId}?action=unpublish`)
        .then(() => {
          meetupData.published_at = null;
  ...
  const deleteMeetup = () => {
    axiosInstance
      .delete(`api/admin/all-meeetups/${meetupId}`)
      .then(() => {
        setDeleteModalVisible(false);
        dispatch(replace('../../../admin/meetups'));
      })
      .catch(() => {
        setDeleteModalVisible(false);
      });
          ...
          {!loading ? (
            <Row wrap={false}>
              <div style={{ width: '612px', minWidth: '612px' }}>
                <EventDetailsComponent type="meetup" data={meetupData} />
              ...
              <InfoSection className="white-text bodyBold">
                <Row>
                  <div className="label">Meetup ID</div>
                  <div>{meetupId}</div>
                </Row>
                <Row>
                ...
                <Row>
                  ...
                  <div
                    className={
                      meetupData.published_at ? 'green-text' : 'error-text'
                    }
                  >
                    {meetupData.published_at
                      ? moment
                          .utc(meetupData.published_at)
                          .local()
                          .format('DD/MM/yyyy HH:mm')
                      : 'Unpublished'}
                  </div>
                ...
                <Row>
                  ...
                  <div>{meetupData.slots}</div>
                </Row>
                <Row wrap={false}>
                  <div className="label">VC Link</div>
                  <div>{meetupData.virtual_room_link || '-'}</div>
                </Row>
                <Row wrap={false}>
                  <div className="label">Recording Link</div>
                  <div>{meetupData.recording_link || '-'}</div>
                </Row>
                <Row>
                  <div className="label">Featured</div>
                  <div>{meetupData.is_featured === 1 ? 'Yes' : 'No'}</div>
                </Row>
                <Row wrap={false}>
                  <div className="label">Notification Message</div>
                  <div>
                    {meetupData.notification_message
                      ? meetupData.notification_message
                      : '-'}
                  </div>
                </Row>
                {!meetupData.deleted_at && (
                  <>
                ...
                <ConfirmationPopupComponent
                  ...
                  title="Delete Meetup"
                  message="This action cannot be undone. Do you wish to delete this meetup?"
                  leftAction={deleteMeetup}
                ...
                <ConfirmationPopupComponent
                  ...
                  title="Unpublish Meetup"
                  message="Do you wish to unpublish this meetup?"
                  leftAction={() => {
                    unpublish();
                ...
                <ConfirmationPopupComponent
                  ...
                  message="This will be posted on the Activity Feed and Meetup List which can be viewed by all Live to Give users"
                ...
                <ConfirmationPopupComponent
                  ...
                  message="The meetup has been published!"
                  actionRequire={false}
                />
              </InfoSection>
            </Row>
